...
@section('content-header')
	<h1> {{ trans('content::custom_contentstories.title.custom_contentstories') }} </h1>
	<ol class="breadcrumb">
				...
				<div class="box-body">
					<div class="table-responsive">
						<table class="data-table table table-bordered table-hover">
							...
							<tbody>
								<?php if (isset($customStories)): ?>
									<?php foreach ($customStories as $customStory): ?>
										<tr>

											<!-- <td>
													<input class="checkbox" type="checkbox" onchange="changed(this);" name="check[]" value="{{ $customStory->id }}"> 
											</td> -->
											<td> {{ $customStory->id }} </td>
											<td> {{ $customStory->title }} </td>
											<td>
												<a href="{{ route('admin.content.custom_contentstory.edit', [$customStory->id]) }}">
													{{ $customStory->created_at }}
												</a>
											</td>
											<td>
												<div class="btn-group">
													<a href="{{ route('admin.content.custom_contentstory.edit', [$customStory->id]) }}" class="btn btn-default btn-flat"><i class="fa fa-pencil"></i></a>
													<button class="btn btn-danger btn-flat" data-toggle="modal" data-target="#modal-delete-confirmation" data-action-target="{{ route('admin.content.custom_contentstory.destroy', [$customStory->id]) }}"><i class="fa fa-trash"></i></button>
												</div>
											</td>
										</tr>
									<?php endforeach; ?>
